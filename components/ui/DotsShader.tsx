...
function createBuffer(gl: WebGL2RenderingContext, arr: Float32Array | Uint16Array | Uint32Array) {
...
const DotsShader = (props: DotsShaderProps) => {
  ...
  const uniforms = useMemo(() => {
    ...
    return {
      u_colors: {
        value: e.map((c) => [c[0] / 255, c[1] / 255, c[2] / 255]),
        type: "uniform3fv",
      },
      u_opacities: {
        value: opacities,
  ...
  useEffect(() => {
    ...
    if (!gl.getProgramParameter(glProgram, gl.LINK_STATUS)) {
      console.error(
        `Failed to compile WebGL program: \n\n${gl.getProgramInfoLog(glProgram)}`
      );
      return;
    }
